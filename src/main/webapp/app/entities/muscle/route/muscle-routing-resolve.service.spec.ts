jest.mock('@angular/router');

import { TestBed } from '@angular/core/testing';
import { HttpResponse } from '@angular/common/http';
import { HttpClientTestingModule } from '@angular/common/http/testing';
import { ActivatedRouteSnapshot, Router } from '@angular/router';
import { of } from 'rxjs';

import { IMuscle, Muscle } from '../muscle.model';
import { MuscleService } from '../service/muscle.service';

import { MuscleRoutingResolveService } from './muscle-routing-resolve.service';

describe('Service Tests', () => {
  describe('Muscle routing resolve service', () => {
    let mockRouter: Router;
    let mockActivatedRouteSnapshot: ActivatedRouteSnapshot;
    let routingResolveService: MuscleRoutingResolveService;
    let service: MuscleService;
    let resultMuscle: IMuscle | undefined;

    beforeEach(() => {
      TestBed.configureTestingModule({
        imports: [HttpClientTestingModule],
        providers: [Router, ActivatedRouteSnapshot],
      });
      mockRouter = TestBed.inject(Router);
      mockActivatedRouteSnapshot = TestBed.inject(ActivatedRouteSnapshot);
      routingResolveService = TestBed.inject(MuscleRoutingResolveService);
      service = TestBed.inject(MuscleService);
      resultMuscle = undefined;
    });

    describe('resolve', () => {
      it('should return IMuscle returned by find', () => {
        // GIVEN
        service.find = jest.fn(id => of(new HttpResponse({ body: { id } })));
        mockActivatedRouteSnapshot.params = { id: 123 };

        // WHEN
        routingResolveService.resolve(mockActivatedRouteSnapshot).subscribe(result => {
          resultMuscle = result;
        });

        // THEN
        expect(service.find).toBeCalledWith(123);
        expect(resultMuscle).toEqual({ id: 123 });
      });

      it('should return new IMuscle if id is not provided', () => {
        // GIVEN
        service.find = jest.fn();
        mockActivatedRouteSnapshot.params = {};

        // WHEN
        routingResolveService.resolve(mockActivatedRouteSnapshot).subscribe(result => {
          resultMuscle = result;
        });

        // THEN
        expect(service.find).not.toBeCalled();
        expect(resultMuscle).toEqual(new Muscle());
      });

      it('should route to 404 page if data not found in server', () => {
        // GIVEN
        spyOn(service, 'find').and.returnValue(of(new HttpResponse({ body: null })));
        mockActivatedRouteSnapshot.params = { id: 123 };

        // WHEN
        routingResolveService.resolve(mockActivatedRouteSnapshot).subscribe(result => {
          resultMuscle = result;
        });

        // THEN
        expect(service.find).toBeCalledWith(123);
        expect(resultMuscle).toEqual(undefined);
        expect(mockRouter.navigate).toHaveBeenCalledWith(['404']);
      });
    });
  });
});
