...
export class MeasureTypeComponent implements OnInit {
  ...
  trackId(index: number, item: IMeasureType): number {
    return item.id!;
  }
  ...
  delete(measureType: IMeasureType): void {
    const modalRef = this.modalService.open(MeasureTypeDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IMeasureType[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/measure-type'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
